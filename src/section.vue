        ...
        <div class="section-container">
            ...
            <div class="content">
                ...
                <ul class="left-content">
                    <li class="feature-wrapper" v-for="(feature, index) in section.data.features" :key="feature.uniqueId">
                        <!-- wwManager:start -->
                        <wwContextMenu v-if="editMode" class="ww-orange-button" tag="div" :options="elemOptions" @remove="removeFeature(feature)" @addBefore="addFeatureBefore(index)" @addAfter="addFeatureAfter(index)">
                        ...
                        <div class="feature-title" :class="{ selected: isFeatureSelected(feature,selectedFeature) }" @click="toggleFeature(feature)">
                            <svg width="12" height="8" viewBox="0 0 12 8" xmlns="http://www.w3.org/2000/svg" class="toggle-icon">
                                <path d="M1.41 0.589966L6 5.16997L10.59 0.589966L12 1.99997L6 7.99997L0 1.99997L1.41 0.589966Z" />
                            </svg>
                            <wwObject tag="span" :ww-object="feature.title"></wwObject>
                        </div>
                        <wwLayoutColumn tag="div" ww-default="ww-text" :ww-list="feature.contentList" @ww-add="add(feature.contentList, $event)" @ww-remove="remove(feature.contentList, $event)">
                            <wwObject tag="div" ww-default="ww-text" class="feature-content" :class="{ selected: isFeatureSelected(feature,selectedFeature) }" v-for="item in feature.contentList" :key="item.uniqueId" :ww-object="item"></wwObject>
                        </wwLayoutColumn>
                    </li>
                </ul>
                <div class="right-wrapper">
                    <wwObject class="background" :ww-object="section.data.rightBackground" ww-category="background"></wwObject>
                    <transition name="fade">
                        <div class="feature-display-wrapper">
                            <wwObject class="feature-display-content" v-for="(feature, index) in section.data.features" :key="index" :class="{active:isFeatureSelected(feature,selectedFeature)}" :ww-object="feature.display"></wwObject>
                        </div>
                    </transition>
                </div>
            </div>
